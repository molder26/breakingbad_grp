...
export function getCharacters(query, pagina){
    return function (dispatch){
        return fetch(`http://localhost:3001/characters?name=${query}&pagina=${pagina}`) // "https://www.breakingbadapi.com/api/characters?name="+query aca estaba el error que vimos en el repaso tenia mal puesta la url(tenia:https://www.breakingbadapi.com/characters?name="+query)
        .then(res => res.json())
        .then(json => {
